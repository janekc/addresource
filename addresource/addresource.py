"""Adds python dependencies as resources to formula."""
...
def get_sha256_hash_from_link(sha256_link, package):
...
def get_latest_source_distribution_and_sha256_hash(package):
    url = f"https://pypi.org/project/{package}/#files"
    soup = get_soup_from_url(url)

    # Find all links that match the source distribution pattern
    source_links = soup.find_all('a', href=re.compile(rf'{package}-.*\.tar\.gz$'))

    if not source_links:
        print(f"No source distribution found for package: {package}")
        return None
    ...
    latest_sha256_hash = get_sha256_hash_from_link(latest_sha256_link, package)
...
def add_resource_to_homebrew_formula(formula_path, package, dry_run):
    ...
    resource_block = f'  resource "{package}" do\n' \
